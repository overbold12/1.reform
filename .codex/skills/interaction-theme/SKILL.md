...
단, 휴대폰번호나 주민등록번호 입력처럼 유효한 입력이 완료된 뒤에만
의미가 생기는 단독 진행 CTA는 이 프로젝트에서 미완료 상태의 disabled 버튼을
미리 노출하지 않는다. 빈 값 또는 입력 중에는 CTA를 렌더링하지 않고,
유효 길이와 조건이 충족되는 순간에만 CTA를 표시한다.
