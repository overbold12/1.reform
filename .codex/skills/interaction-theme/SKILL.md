...
체크 표시를 사용하는 원형 선택 UI는 선택 여부와 관계없이 체크 기호를 유지한다.

- 미선택 상태: 흰 배경, 연한 회색 테두리, 연한 회색 체크 표시
- 선택 상태: 강조색 배경과 테두리, 흰색 체크 표시
- 미선택 상태를 체크 기호가 없는 빈 원으로 표현하지 않는다.

이 규칙은 체크박스, 약관 동의, 혜택 선택, 상환방식 등 원형 체크 UI에 공통 적용한다.
